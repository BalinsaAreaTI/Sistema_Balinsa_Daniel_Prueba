...
class CventaController extends Controller
{
    public function index()
    {
        ...
        // Roles que pueden ver todas las cotizaciones
        $rolesConAccesoTotal = ['Area de TI', 'Gerencia', 'Administracion', 'Asesor de Ventas'];

        // Usuarios específicos que solo pueden ver sus propias cotizaciones
        $usuariosRestringidos = ['Yanina Cordova Giron', 'Brandoock Garcia Trelles'];

        if ($user->roles->pluck('name')->intersect($rolesConAccesoTotal)->isNotEmpty()) {
            // Verifica si el usuario está en la lista de restringidos
            if (in_array($nombreCompleto, $usuariosRestringidos)) {
                // Solo mostrar cotizaciones del usuario restringido
                $cventas = Cventa::with('cliente')
                    ->where('tecnico', $nombreCompleto)
                    ->orderBy('id', 'DESC')
                    ->paginate(30);
            } else {
                // Usuario con rol permitido ve todas las cotizaciones
                $cventas = Cventa::with('cliente')
                    ->orderBy('id', 'DESC')
                    ->paginate(30);
            }
        } else {
            // Otros usuarios solo ven sus propias cotizaciones
            $cventas = Cventa::with('cliente')
                ->where('tecnico', $nombreCompleto)
                ->orderBy('id', 'DESC')
                ->paginate(30);
