...
// Variable reactiva para almacenar el id del producto que tiene el mouse encima
const hoveredId = ref(null);

// Función para obtener la fecha más reciente entre created_at y fecha_act
const getMostRecentDate = (createdAt, fechaActInput) => {
  const created = createdAt ? new Date(createdAt) : null;
  const fechaAct = fechaActInput ? new Date(fechaActInput) : null;

  // Comparar ambas fechas y retornar la más reciente
  if (created && fechaAct) {
    return created > fechaAct ? created : fechaAct; // Fecha más reciente
  }

  // Si alguna de las fechas está vacía, devolver la que tenga datos
  return created || fechaAct;
};
</script>
    ...
    <AppLayout title="Catalago Productos">
                            ...
                            <div class="relative overflow-x-auto scroll-dataTableLEO shadow-md md:rounded-lg rounded-md shadow-gray-200 dark:shadow-gray-500">
                                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-900">
                                    <thead class="text-xs text-white uppercase bg-green-600 dark:bg-green-600">
                                        <tr>
                                            <th scope="col" class="px-6 py-3 text-center dark:border-white border-b-2">Item</th>
                                            ...
                                            <th scope="col" class="px-6 py-3 text-center dark:border-white border-b-2" v-if="$page.props.user.permissions.includes('Acciones Administrador')">P_Proveedor</th>
                                    ...
                                    <tbody>
                                        <tr v-for="(tbproducto, i) in filteredTbproductos" :key="tbproducto.id" class="bg-white text-black dark:bg-gray-700 dark:text-white">
                                            <td class="px-6 py-4 text-center hidden">{{ tbproducto.id }}</td>
                                            ...
                                            <td class="px-6 py-4 text-center border-r border-b font-semibold">{{ tbproducto.modelo }}</td>
                                            <td class="px-6 py-4 text-center border-r border-b font-extrabold whitespace-nowrap">{{ tbproducto.medida }}</td>
                                            ...
                                            <td class="md:px-2 px-2 py-2 text-center border-r border-b">
                                                <img @click="openModal('/productos_img/' + tbproducto.foto)"
                                                    ...
                                                    <a class="font-extrabold text-blue-900 dark:text-gray-50">{{ tbproducto.proveedor ? tbproducto.proveedor : 'S/N' }}</a>
                                            </td>
                                            ...
                                            <td
                                                v-if="$page.props.user.permissions.includes('Acciones Administrador')" class="px-6 py-4 text-center border-r border-b relative cursor-pointer"
                                                @mouseover="hoveredId = tbproducto.id"
                                                @mouseleave="hoveredId = null"
                                            >
                                                <!-- Mostrar el precio -->
                                                {{ tbproducto.precio_list ? `${tbproducto.moneda} ${tbproducto.precio_list}` : 'Por asignar' }}

                                                <!-- Mostrar la fecha más reciente solo para la celda seleccionada -->
                                                <span v-if="hoveredId === tbproducto.id" class="tooltip absolute bg-gray-800 text-white text-xs px-2 py-1 rounded shadow-lg z-50 mt-2">
                                                    Precio actualizado al: {{ getMostRecentDate(tbproducto.created_at, tbproducto.fecha_act)?.toLocaleDateString('es-ES', { timeZone: 'UTC' }) }}
                                                </span>
                                            </td>
                                            ...
                                            <td class="p-3 text-center border-b" v-if="$page.props.user.permissions.includes('Acciones Catalago')">
                                                <Link v-if="$page.props.user.permissions.includes('Editar Catalago')" :href="route('tbproductos.edit', { tbproducto: tbproducto.id })" class="inline-flex items-center justify-center bg-amber-400 hover:bg-amber-500 px-1.5 py-0.5 rounded-md mr-2">
                                                    <i class='bx bxs-edit text-base text-white'></i>
                                                </Link>
                                                <button v-if="$page.props.user.permissions.includes('Eliminar Catalago')" @click="$event => deleteTbproducto(tbproducto.id, tbproducto.modelo)" class="inline-flex items-center justify-center bg-red-600 hover:bg-red-700 px-1.5 py-0.5 rounded-md">
        ...
        <div class="fixed right-0 top-[110px] bottom-[-60px] md:left-[50px] left-[-2px] overflow-y-auto z-[10000] bg-gray-200/40" v-if="modalOpen">
            <div class="flex justify-center items-center flex-col h-full max-h-[90%] py-10 m-auto rounded-lg" style="backdrop-filter: blur(5px);" @click="modalOpen = false">
                <div class="w-full max-w-sm max-h-[90%] h-full bg-gray-50 dark:bg-gray-600 rounded-t-lg">
                    <img :src="modalImageUrl" alt="Imagen ampliada" class="max-w-sm w-full h-full object-contain">
                </div>
                <!-- Botón de cierre del modal -->
                <div class="bg-gray-100 p-2 w-full max-w-sm flex justify-end rounded-b-lg dark:bg-gray-700">
...
.scroll-dataTableLEO thead {
    position: sticky;
    top: 0;
    background-color: #16A34A; /* Ajusta el color para que coincida con tu diseño */
    z-index: 10;
}
