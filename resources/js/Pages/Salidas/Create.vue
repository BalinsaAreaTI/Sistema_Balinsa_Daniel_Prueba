...
const selectUsuarios = ref(null); // Valor seleccionado
...
watch(selectUsuarios, (newValue) => {
    form.tecnico = newValue; // Update the technician ID in the form
});

const submit = async (dataToSubmit) => {
    try {
        const response = await axios.post('/salidas.store', dataToSubmit);
...
const submitForm = (event) => {
    ...
    const { producto_id, unidad_salida } = form;
...
onMounted(() => {
    ...
    form.hora_salida = `${hours}:${minutes}`;
...
<template>
    <AppLayout title="Crear Salida">
        <template #header>
            <h1 class="font-semibold text-base uppercase text-gray-800 leading-tight dark:text-white">Registrar Salida</h1>
        </template>
                                    ...
                                    <div class="">
                                        ...
                                        <div class="card text-sm mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
                                            <div>
                                                ...
                                                <InputNumber v-model="form.unidad_salida" inputId="integeronly" fluid />
                                            ...
                                            <div class="flex justify-between gap-x-20 items-center text-center w-full">
                                                <div class="flex justify-between items-center gap-x-2.5">
                                                    ...
                                                    <div class="flex items-center">
                                                        <input checked id="inline-checked-radio" v-model="form.devolucion" type="radio" value="0" name="inline-radio-group" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-blue-800 focus:ring-2 dark:bg-gray-700 dark:border-white">
                                                        <label for="inline-checked-radio" class="ms-2 text-sm font-bold text-gray-900 dark:text-white">NO</label>
                                                    </div>
                                                    <div class="flex items-center">
                                                        <input id="inline-2-radio" v-model="form.devolucion" type="radio" value="1" name="inline-radio-group" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-white">
