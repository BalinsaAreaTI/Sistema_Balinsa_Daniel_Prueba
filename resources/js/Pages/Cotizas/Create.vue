            ...
            doc.autoTable({
                ...
                body: [
                    ...datosTabla.map(product => {
                        // Inicializa el código del producto
                        let codProducto = '';

                        // Si necesitas realizar un recorrido sobre las filas de la tabla,
                        // pero parece que no estás usando 'modedaInversa' en tu lógica actual.
                        const rows = document.querySelectorAll('.relative table tbody tr');
                        rows.forEach(function (row) {
                            const modedita = row.children[14];
                            if (modedita) {
                                const modedaInversa = modedita.innerText.trim();
                                console.log('Moneda:', modedaInversa);
                                // Aquí podrías hacer algo con modedaInversa si es necesario.
                            }
                        });
                        ...
                        const invertedPrice = Math.round(product.precio_list);
                        const invertedString = invertedPrice
                            .toString()
                            .split('')
                            .reverse()
                            .join('');

                        // Asignar el código del producto según la moneda
                        if (product.moneda === "$") {
                            codProducto = `Cod Producto: B02 - ${invertedString}`;
                        } else if (product.moneda === "s/") {
                            codProducto = `Cod Producto: B01 - ${invertedString}`;
                        } else {
                            codProducto = `Cod Producto: Desconocido - ${invertedString}`; // Manejar caso desconocido
                        }

                        // Unir especificaciones y código del producto
                        const fullSpecifications = `${especificacionesList}\n${codProducto}`;
                    ...
                    <div class="h-full mx-auto px-4 sm:px-6 lg:px-8">
                      ...
                                </div>
                                ...
                                <div>
                                    ...
                                    <TextInput type="date" :disabled="!$page.props.user.permissions.includes('Editar Fecha Servicio')" id="fecha" v-model="form.fecha" required
                                        class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                                    ...
                                    <table class="w-full text-sm text-left rtl:text-right text-black dark:text-white">
                                        <thead class="text-xs text-white uppercase bg-green-600 dark:bg-green-600">
                                            <tr>
                                                ...
                                                <th scope="col" class="px-6 py-3 text-center dark:border-white border-b-2 hidden">P. Proveedor</th>
                                        ...
                                        <tbody>
                                            <tr v-for="(tbproducto, i) in tbproductosAgregados" :key="i">
                                                ...
                                                <td class="px-6 py-4 text-center border-r whitespace-nowrap border-b hidden">{{ tbproducto.moneda }} {{ parseFloat(tbproducto.precio_list ? tbproducto.precio_list : '0').toFixed(2) }}</td>
                                ...
                                <div class="flex w-full">
                                    <div class="w-full">
                                        ...
                                        <select id="moneda" v-model="form.moneda" required
                                                class="h-10 dark:text-black focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-l-md">
                        ...
                        <table class="w-full text-sm text-left rtl:text-right text-black dark:text-white">
                            <thead class="text-xs text-white uppercase bg-green-600 dark:bg-green-600">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center border-b-2 border-white">Medida</th>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-center border-b-2 border-white" v-if="$page.props.user.permissions.includes('Acciones Administrador')">P. Proveedor</th>
                            ...
                            <tbody>
                                <tr v-for="tbproducto in filteredProductos" :key="tbproducto.id" class="bg-white text-black dark:bg-gray-700 dark:text-white">
                                    ...
                                    <td class="px-6 py-3 text-center border-r border-b whitespace-nowrap">{{ tbproducto.medida }}</td>
                                    <td class="px-6 py-4 text-center border-r border-b whitespace-nowrap">
                                        <div class="accordions">
                                            <dl>
                                    ...
                                    <td class="px-6 py-4 text-center border-r border-b" v-if="$page.props.user.permissions.includes('Acciones Administrador')">{{ tbproducto.moneda }} {{ parseFloat(tbproducto.precio_list ? tbproducto.precio_list : '0').toFixed(2) }}</td>
