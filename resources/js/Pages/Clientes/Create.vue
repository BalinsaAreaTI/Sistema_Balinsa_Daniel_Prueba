...
const save4 = () => {
    if (operation4.value == 1) {
        form4.post(route('tbprovincias.store'), {
            onSuccess: () => {
                ok4('La ciudad ha sido registrada correctamente', 'success');
                $.ajax({
                    url: '/fn_obtenerProvinciasRegistradas',
                    method: 'GET',
                    success: function(response) {
                        provincias.value = response;
                        filteredClientes.value = provincias.value.filter(prov => prov.prov_nombre.toLowerCase().includes(searchTerm.value.toLowerCase()));
                    }
                });
            }
        });
                                ...
                                <div>
                                    ...
                                    <div class="w-full flex justify-end items-end">
                                        <TextRuc v-model="form.numeroDocumento" type="text" id="numeroDocumento" placeholder="Ingrese y consulte el RUC"
                                         class="mt-2 w-full dark:text-black"/>
                                    ...
                                    <div class="w-full">
                                        ...
                                        <div class="relative">
                                            <TextInputCliente v-model="searchTerm" autocomplete="off" type="text" id="cliente_id" @input="onInput" placeholder="Ingresa nombre de ciudad"
                                                ...
                                                class="mt-1 focus:ring-indigo-500 dark:text-black focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-l-lg h-[41.6px]" />
